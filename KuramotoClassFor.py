...
class Kuramoto:
    ...
    def IntegrateDD(self):
        ...
        if n < 20:
            ...
            chunksize = 50   # was 4 — this matters a lot
        else:
            chunksize = 80
        time_start=tm.time()

        rtol,atol = 1e-4,1e-4 ##felipe values
        ...
        if self.Forced:
            ...
            DDE.set_integration_parameters(rtol=rtol, atol=atol,pws_max_iterations=1)
            DDE.constant_past(random.uniform(0, 2*np.pi, n), time=0.0)
            if max_delay>0:
                DDE.integrate_blindly(max_delay, dt*1.0e-2)
        
        elif not self.Forced:
            DDE = jitcdde(self.kuramotos, n=n,delays=self.delays_matrix.flatten(),verbose=True)


            DDE.compile_C(simplify=False, do_cse=False, chunk_size=chunksize,verbose=True) ###################hard-coded chunk size?
            DDE.set_integration_parameters(rtol=rtol, atol=atol,pws_max_iterations=1)
            DDE.constant_past(random.uniform(0, 2*np.pi, n), time=0.0)

            print(f"compiled and running, max_delay = {max_delay}")
            if max_delay>0:
